// mockData.ts - Complete mock data for Travel Smart (TypeScript)
...
export const experienceTypes: ExperienceType[] = [
  { id: 'beach', label: 'Beach Relaxation', icon: '🏖️', description: 'Sun, sand, and sea' },
  { id: 'city', label: 'City Exploration', icon: '🏙️', description: 'Museums, culture, and urban life' },
  { id: 'adventure', label: 'Adventure', icon: '⛰️', description: 'Hiking, sports, and thrills' },
  { id: 'cultural', label: 'Cultural Heritage', icon: '🏛️', description: 'History and traditions' },
  { id: 'romantic', label: 'Romantic Getaway', icon: '💑', description: 'Perfect for couples' },
  { id: 'nightlife', label: 'Nightlife & Entertainment', icon: '🎉', description: 'Bars, clubs, and events' }
];
...
export const activityLevels: ActivityLevel[] = [
  { id: 'relaxation', label: 'Pure Relaxation', description: 'Minimal physical activity' },
  { id: 'light', label: 'Light Activities', description: 'Easy walks and sightseeing' },
  { id: 'moderate', label: 'Moderate', description: 'Mix of activities and rest' },
  { id: 'high', label: 'High Adventure', description: 'Active and energetic' }
];

export const budgetRanges: BudgetRange[] = [
  { id: 'economy', label: 'Economy', range: '£500-1000', icon: '💰' },
  { id: 'moderate', label: 'Moderate', range: '£1000-2500', icon: '💰💰' },
  { id: 'luxury', label: 'Luxury', range: '£2500+', icon: '💰💰💰' }
];

export const durations: Duration[] = [
  { id: 'weekend', label: 'Weekend Getaway', days: 3 },
  { id: 'short', label: 'Short Break', days: 5 },
  { id: 'week', label: 'One Week', days: 7 },
  { id: 'extended', label: 'Extended Stay', days: 10 }
];

export const flightDistances: FlightDistance[] = [
  { id: 'short', label: 'Short Haul', hours: '2-4 hours' },
  { id: 'medium', label: 'Medium Distance', hours: '4-7 hours' },
  { id: 'long', label: 'Long Haul', hours: '7+ hours' }
];
 ...
 * Returns top 2 matching destinations sorted by confidence score
 */
export function getRecommendations(userPreferences: UserPreferences): Destination[] {
  const {
    experienceType,
    travelCompanion,
    activityLevel,
  ...
  const filtered = destinations.filter(dest => {
    ...
    if (!dest.tags.includes(experienceType)) return false;
    
    // Match travel companion
    if (!dest.bestFor.includes(travelCompanion)) return false;
    
    // Match activity level
    if (!dest.activityLevel.includes(activityLevel)) return false;
    
    // Match budget
    if (dest.budgetCategory !== budgetRange) return false;
    
    // Match flight distance
    if (dest.flightDistance !== flightDistance) return false;
    
    // Check child-friendly requirement
    if (specialRequirements.includes('childFriendly') && !dest.childFriendly) {
      return false;
    }
 ...
 */
export function getDestinationsByExperience(experienceType: string): Destination[] {
